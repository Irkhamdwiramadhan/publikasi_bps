<x-app-layout>
                ...
                <div class="card-body p-6 md:p-8">
                    ...
                    <form action="{{ route('spnsr.store') }}" method="POST">
                        ...
                        <div class="space-y-6">
                            ...
                            <section>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="form-control w-full">
                                        <label for="nomor_surat_prefix" class="block text-sm font-semibold text-base-content/80 mb-1">Nomor Surat</label>

                                        {{-- Kita gunakan DaisyUI 'join' untuk menggabungkan input secara visual --}}
                                        <div class="join w-full">

                                            {{-- 1. Input yang diisi oleh user --}}
                                            <input id="nomor_surat_prefix"
                                                type="text"
                                                placeholder="Contoh: B-262"
                                                class="input input-bordered join-item w-1/3 {{ $errors->has('nomor_surat') ? 'input-error' : '' }}"
                                                required>

                                            {{-- 2. Bagian Statis (read-only). Kita buat sebagai input 'disabled' agar terlihat menyatu --}}
                                            {{-- Perhatikan: Saya membuat tahun '2025' menjadi dinamis menggunakan {{ date('Y') }} --}}
                                            <input type="text"
                                                id="nomor_surat_suffix_display"
                                                value="/3328/HM.600/{{ date('Y') }}"
                                                class="input input-bordered join-item w-2/3"
                                                disabled>
                                        </div>

                                        {{-- 3. Input Tersembunyi (hidden) yang nilainya akan dikirim ke Controller --}}
                                        <input type="hidden"
                                            id="nomor_surat"
                                            name="nomor_surat"
                                        ...
                                        <small class="text-xs text-base-content/60 mt-1">
                                            <b>Penting:</b> Anda hanya perlu mengisi bagian awal nomor (misal: B-262).
                                        </small>
                                    </div>
                                    ...
                                    <div class="form-control w-full">
                                        ...
                                        <textarea id="tanggal_prosa" name="tanggal_prosa" rows="3" class="textarea textarea-bordered w-full rounded-[15px] {{ $errors->has('tanggal_prosa') ? 'textarea-error' : '' }}" required placeholder="">{{ old('tanggal_prosa') }}</textarea>
                                        <small class="text-xs text-base-content/60 mt-1">
                                            <b>Penting:</b> Masukkan kalimat tanggal lengkap seperti contoh Pada hari Kamis tanggal dua puluh tujuh bulan Februari tahun dua ribu dua puluh lima.
                                        </small>
                                        @error('tanggal_prosa')
                                        <p class="text-error text-xs mt-2">{{ $message }}</p>
                                        @enderror
                            ...
                            {{-- Tambahkan ini (misalnya setelah 'tanggal_prosa') --}}

                            <div class="form-control w-full">
                                <label for="tanggal_rilis" class="block text-sm font-semibold text-base-content/80 mb-1">Tanggal Rilis Publikasi</label>
                                <input id="tanggal_rilis" type="date" name="tanggal_rilis" value="{{ old('tanggal_rilis') }}" class="input input-bordered w-full rounded-[15px] {{ $errors->has('tanggal_rilis') ? 'input-error' : '' }}" required>
                                @error('tanggal_rilis')
                                <p class="text-error text-xs mt-2">{{ $message }}</p>
                                @enderror
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const prefixInput = document.getElementById('nomor_surat_prefix');
            const suffixDisplay = document.getElementById('nomor_surat_suffix_display');
            const hiddenFullInput = document.getElementById('nomor_surat');

            // Ini adalah bagian statisnya, misal: "/3328/HM.600/2025"
            const staticSuffix = suffixDisplay.value;

            // Fungsi untuk menggabungkan dan memperbarui input tersembunyi
            function updateHiddenInput() {
                const prefixValue = prefixInput.value;
                hiddenFullInput.value = prefixValue + staticSuffix;
            }

            // 1. Logika untuk menangani 'old' input (jika validasi gagal)
            // Ini penting agar user tidak perlu mengetik ulang
            const oldFullValue = "{{ old('nomor_surat') }}";
            if (oldFullValue) {
                // Cek apakah old value mengandung suffix statis kita
                if (oldFullValue.endsWith(staticSuffix)) {
                    // Jika ya, pisahkan dan isi 'prefix' input
                    prefixInput.value = oldFullValue.substring(0, oldFullValue.length - staticSuffix.length);
                } else {
                    // Jika tidak (misal data lama tidak lengkap), isi saja apa adanya
                    prefixInput.value = oldFullValue;
                }

                // Set nilai awal input tersembunyi
                hiddenFullInput.value = oldFullValue;
            }

            // 2. Event listener utama
            // Setiap kali user mengetik di kotak prefix...
